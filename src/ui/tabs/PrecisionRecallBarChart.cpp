#include "core/PrecisionRecallBarChart.h"
...
void PrecisionRecallBarChart::setData(const QVector<ClassMetric>& metrics)
{
    m_metrics = metrics;
    updateChart();
    // createGroupedChart(metrics);
}
...
void PrecisionRecallBarChart::updateChart()
{
    ...
    {
        *precisionSet << metric.precision; // 转换为百分比
        *recallSet << metric.recall;
        if(f1Set)
...
void PrecisionRecallBarChart::createGroupedChart(const QVector<ClassMetric>& metrics)
{
    m_chart->removeAllSeries();
    m_chart->removeAxis(m_chart->axisX());
    m_chart->removeAxis(m_chart->axisY());
    ...
    {
        // 创建精确率条形
        QBarSet* precisionBar = new QBarSet(metric.name);
        precisionBar->setLabel("精确率");
        precisionBar->append(metric.precision * 100);
        precisionBar->setColor(QColor(65,105,225)); // 蓝色

        // // 创建召回率条形
        // QBarSet* recallBar = new QBarSet(metric.name);
        // recallBar->setLabel("召回率");
        // recallBar->append(metric.recall * 100);
        // recallBar->setColor(QColor(220, 20, 60));   // 红色

        QBarSet* f1Bar = new QBarSet(metric.name);
        f1Bar->setLabel("f1分数");
        f1Bar->append(metric.f1Score * 100);
        f1Bar->setColor(QColor(50, 205, 50));   // 绿色

        // 创建分组系列
        QBarSeries* groupSeries = new QBarSeries();
        groupSeries->append(precisionBar);
        // groupSeries->append(recallBar);
        groupSeries->append(f1Bar);

        // 添加到图表
        m_chart->addSeries(groupSeries);
    }

    // 创建坐标轴
    QBarCategoryAxis* axisX = new QBarCategoryAxis();
    axisX->append({"Crazing", "Patches", "Inclusion"});
    m_chart->addAxis(axisX, Qt::AlignBottom);
